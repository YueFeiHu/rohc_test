...

{
	...
	rohc_debugf(2, "send_static = %d, send_dynamic = %d\n", ip_profile->tmp_variables.send_static, ip_profile->tmp_variables.send_dynamic);
	// 3
...
void decide_state(struct sc_context *context)
{
	...
	if (context->c_state == IR && send_dynamic && ip_profile->ir_count >= MAX_IR_COUNT)
	{
		change_state(context, FO);
	}
	else if (context->c_state == IR && send_static && ip_profile->ir_count >= MAX_IR_COUNT)
	{
		change_state(context, FO);
	}
	...
	else if (context->c_state == FO && send_dynamic && ip_profile->fo_count >= MAX_FO_COUNT)
	{
		change_state(context, FO);
	}
	else if (context->c_state == FO && send_static && ip_profile->fo_count >= MAX_FO_COUNT)
	{
		change_state(context, FO);
	}
...
int decide_FO_packet(struct sc_context *context)
{
	...
	else if (ip_profile->tmp_variables.send_static)
	{ // if static field changed we must go back to ir
		return c_UO2;
	}
...
int decide_SO_packet(const struct sc_context *context)
{
	...
	else
	{
		...
		else if (nr_sn_bits <= 4 && (context->rnd == 1 || nr_ip_id_bits == 0) && (context->rnd2 == 1 || nr_ip_id_bits2 == 0))
		{
					 ...
					 int counter)
{
	...
	rohc_debugf(3, "code_IR, protocol=%d\n", dest[counter]);
	counter++;
...
int decide_extension(struct sc_context *context)
{
	...
	else
	{
		...
		if (nr_sn_bits < 5 && (nr_ip_id_bits == 0 || context->rnd == 1) && (nr_ip_id_bits2 == 0 || context->rnd2 == 1))
		{
			return c_NOEXT;
		}
		else if (nr_sn_bits <= 8 && nr_ip_id_bits <= 3 && (nr_ip_id_bits2 == 0 || context->rnd2 == 1))
		{
			return c_EXT0;
		}
		else if (nr_sn_bits <= 8 && nr_ip_id_bits <= 11 && (nr_ip_id_bits2 == 0 || context->rnd2 == 1))
		{
			return c_EXT1;
		}
		else if (nr_sn_bits <= 3 && nr_ip_id_bits <= 11 && nr_ip_id_bits2 <= 8)
		{
			return c_EXT2;
		}
