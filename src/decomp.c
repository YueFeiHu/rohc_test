...
#include "c_util.h"
#include "feedback.h"
extern struct s_profile d_ip_profile;
...
int d_decode_header(struct sd_rohc *state, unsigned char *ibuf, int isize, unsigned char *obuf, int osize, struct sd_decode_data *ddata)
{
	...
	if (!ddata->addcidUsed && state->medium->cid_type == ROHC_LARGE_CID)
	{ // check if large cids are used
	...
	else
	{
		...
		{ // context is valid
			...
			if (d_is_irdyn(walk))
			{
				...
				{ // if IR-DYN changes profile, make comp. transit to NO_CONTEXT-state
...
int rohc_ir_dyn_packet_crc_ok(unsigned char *walk, const int largecid, const int addcidUsed, const struct s_profile *profile, struct sd_context *context)
{
	...
	if (crc != realcrc)
	{
		rohc_debugf(0, "ROHC Decompress IR_DYN: CRC FAILED! SKA: %i, �: %i\n", realcrc, crc);
		return (0);
	}
