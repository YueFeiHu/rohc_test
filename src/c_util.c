...
void c_add_wlsb(struct sc_wlsb *s, int sn, int time, int value)
{
	...
	s->next++;
	if (s->next >= s->windowWidth)
		s->next = 0;
	if (s->oldest >= s->windowWidth)
		s->oldest = 0;
}
...
int c_get_k_wlsb(struct sc_wlsb *s, int value)
{
	...
	if (valid)
	{
		...
		return -1; // return -1 if no k match
	return k;
}
...
void c_ack_remove(struct sc_wlsb *s, int index, int time)
{
	...
	{ // remove all entries from oldest to wrap-around, and all from start to (excluding) index
...
unsigned char crc_calc_8(unsigned char *buf, int size)
{
	int i;
...
unsigned char crc_calc_7(unsigned char *buf, int size)
{
	int i;
...
unsigned char crc_calc_3(unsigned char *buf, int size)
{
	int i;
...
unsigned int crc_calculate(int type, unsigned char *data, int length)
{
	...
	{
	case CRC_TYPE_8:
		return crc_calc_8(data, length);
	case CRC_TYPE_7:
		return crc_calc_7(data, length);
	case CRC_TYPE_3:
		return crc_calc_3(data, length);
	}
	return 0;
}
