...
void *iponly_allocate_decode_data(void)
{
	...
	memset(p, 0, sizeof(struct s_iponly_profile_data) + 4 * sizeof(struct s_iponly_change));
...
int iponly_detect_ir_size(unsigned char *first_byte, int second_byte_add)
{
	...
	if (d)
		ret += 5 + 2;
	if (first_byte[second_byte_add + 2] != 0x40)
		return 0;
	if (first_byte[second_byte_add + 3] == PROTOCOL_IP_IN_IP)
	{
		...
		if (d)
			ret += 5;
		if (first_byte[second_byte_add + 12] != 0x40)
			return 0;
	}
	return ret;
}
...
int iponly_detect_ir_dyn_size(unsigned char *first_byte, struct sd_context *c)
{
	...
	if (pro->active1->ip.protocol == PROTOCOL_IP_IN_IP)
		return 12;
	return 7;
}
	...
	unsigned char *dest)
{
	...
	if (context->state == ROHC_NO_CONTEXT)
		return ROHC_ERROR;
	...
	case PACKAGE_UO_0:
		...
		if (context->state == ROHC_STATIC_CONTEXT)
			return ROHC_ERROR;
...
struct s_profile d_ip_profile = {4, "1.0", "IP / Decompressor",
										iponly_decode,
										iponly_decode_ir,
